    ...
    try {
      ...
      if (resolverLockResult.rows[0]?.acquired !== true) {
        ...
        return {
         ...
         --------------------------------------------------------
         ACS AIRBUS OCC GLOBAL RULE:

         - A valid persistent A/B plan prevents a false OVERDUE.
         - A live SCHEDULED or IN_PROGRESS occurrence also prevents
           a false OVERDUE.
         - B remains technically dominant over A when B is overdue.
         - C/D remain superior authority over A/B.
         - No plan is deleted or modified in this phase.
         ======================================================== */
          ...
          b_check_status = CASE
            WHEN EXISTS (
              ...
              WHERE b.airline_id = ams.airline_id
                ...
                AND b.event_status = 'IN_PROGRESS'
            )
              THEN 'IN_PROGRESS'
            ...
            WHEN EXISTS (
              SELECT 1
              FROM public.schedule_items si
              WHERE si.airline_id = ams.airline_id
                AND si.aircraft_id = ams.aircraft_id
                AND LOWER(COALESCE(si.item_type, '')) = 'service'
                AND UPPER(COALESCE(si.service_type, '')) = 'B'
            ...
            WHEN
              ams.b_check_due_date IS NOT NULL
              AND ams.b_check_due_date
                  <= acs_get_current_sim_time()
              THEN 'OVERDUE'
          ...
          a_check_status = CASE
            WHEN EXISTS (
              ...
              WHERE a.airline_id = ams.airline_id
                ...
                AND a.event_status = 'IN_PROGRESS'
            )
              THEN 'IN_PROGRESS'
            ...
            WHEN EXISTS (
              ...
              WHERE si.airline_id = ams.airline_id
                AND si.aircraft_id = ams.aircraft_id
                AND LOWER(COALESCE(si.item_type, '')) = 'service'
                AND UPPER(COALESCE(si.service_type, '')) = 'A'
                AND LOWER(COALESCE(si.status, 'scheduled'))
                    <> 'cancelled'
            )
              THEN 'SCHEDULED'
            ...
            WHEN
              ams.a_check_due_date IS NOT NULL
              AND ams.a_check_due_date
                  <= acs_get_current_sim_time()
              THEN 'OVERDUE'
      ...
      for (const event of completionResult.rows) {
        ...
        const statusUpdateResult = await client.query(
          ...
          SET
            a_check_status = CASE
              WHEN $3 IN ('A_CHECK', 'B_CHECK') THEN 'OPEN'
              ELSE a_check_status
            END,
        ...
        /*
         * Create the next technical occurrence from the same plan.
         * The selected weekday/time remains under player authority.
         * The technical due date determines the first eligible week.
         */
        const nextDueDate =
          checkType === 'B_CHECK'
        ...
        if (
          ...
          const nextOccurrenceResult = await client.query(
            ...
            candidate AS (
              SELECT
                plan.*,
                $5::TIMESTAMP AS due_at,
                date_trunc('day', $5::TIMESTAMP)
                + (
                    (
                      plan.selected_iso_day
                      - EXTRACT(ISODOW FROM $5::TIMESTAMP)::INTEGER
                      + 7
                    ) % 7
                  ) * INTERVAL '1 day'
                + plan.start_minute * INTERVAL '1 minute'
                  AS raw_start
              FROM plan
            ...
            SELECT
              ...
              NULL,
              jsonb_build_object(
                'source', 'ACS_AB_RECURRING_PLAN_V1',
                'previous_event_id', $10::BIGINT,
                'technical_due_at', $5::TIMESTAMP,
                'persistent_schedule_item_id', resolved.id
              )::TEXT,
              (CURRENT_TIMESTAMP AT TIME ZONE 'UTC'),
              (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')
            FROM resolved
            WHERE NOT EXISTS (
              SELECT 1
              FROM public.aircraft_maintenance_events open_event
              WHERE open_event.airline_id = resolved.airline_id
                AND open_event.aircraft_id = resolved.aircraft_id
                AND open_event.check_type = $4
                AND open_event.event_status IN (
                  'SCHEDULED',
                  'IN_PROGRESS'
                )
            )
         ...
         * after the new A due date. The A plan is never cancelled.
         */
        if (
          checkType === 'B_CHECK' &&
          technicalState.a_check_due_date
...
AND NOT EXISTS (
  ...
  WHERE active_light.airline_id = ame.airline_id
    ...
    AND active_light.check_type = 'B_CHECK'
)
      ...
      for (const event of dueEventsResult.rows) {
        ...
        /*
         * ACS OCC RULE — B has execution priority but never deletes A.
         * A remains SCHEDULED and will be repositioned by PHASE 1 when
         * B completes and resets the A technical cycle.
         */
