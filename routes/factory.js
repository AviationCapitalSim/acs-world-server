...
/* ============================================================
   GET /v1/aircraft/factory/planning-catalog?year=1940
   ------------------------------------------------------------
   ACS ROUTE PLANNING — HISTORICAL AIRCRAFT STUDY CATALOG

   Rules:
   - Read-only.
   - PostgreSQL authority.
   - All aircraft models already existing by the requested
     simulation year may be studied.
   - Aircraft from future years are excluded.
   - Production end year does NOT remove an aircraft.
   - Ownership does NOT matter.
   - Operational status does NOT matter.
   - No fleet, route, slot, schedule or finance writes.
   ============================================================ */

router.get("/planning-catalog", async (req, res) => {
  try {
    const year = Number(req.query.year);
    ...
    return res.json({
      ok: true,
      endpoint: "ACS_ROUTE_PLANNING_AIRCRAFT_CATALOG",
      version: "v1.0",
      year,
      count: result.rows.length,
      aircraft: result.rows
    ...
    console.error(
      "ACS ROUTE PLANNING AIRCRAFT CATALOG ERROR:",
      err
    );

    return res.status(500).json({
      ok: false,
      error: "ROUTE_PLANNING_AIRCRAFT_CATALOG_FAILED",
      message: err.message
    });
  }
});
